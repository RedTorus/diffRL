...
class QVPO(object):
    ...
    def __init__(self, cfg):
        ...
        if self.policy_type == 'Diffusion':
            self.actor = Diffusion(state_dim=cfg.state_dim, action_dim=action_dim, noise_ratio=cfg.args.noise_ratio,
                                   beta_schedule=cfg.args.beta_schedule, n_timesteps=cfg.args.n_timesteps, mode="qvpo").to(cfg.device)
            ...
            self.cut = cfg.args.cut
            self.epsilon = cfg.args.epsilon
            self.entropy_alpha = cfg.args.entropy_alpha
        ...
        self.actor_target.behavior_sample = cfg.args.target_sample
    ...
    #                                eval_sample=args.eval_sample, deterministic=args.deterministic, mode="qvpo").to(device)
    #         self.running_q_std = 1.0
    #         self.running_q_mean = 0.0
    #         self.beta = args.beta
    ...
    def compute_actor_loss(self, batch):
        # sample or augment actions depending on mode
        if self.aug:
            if self.gradient:
                states, best_actions, qv, (mean, std) = self.aug_gradient(self.cfg.args.batch_size, None, return_mean_std=True)
            else:
                states, best_actions, qv, (mean, std) = self.action_aug(self.cfg.args.batch_size, None, return_mean_std=True)
        else:
        ...
        if self.policy_type.lower() == 'diffusion' and self.weighted:
            ...
            # transform Q to weights
            w = eval(self.q_transform)(
                q, q_neg=self.q_neg, cut=self.cut,
                running_q_std=self.running_q_std,
                running_q_mean=self.running_q_mean,
                beta=self.beta, v=v,
                batch_size=self.cfg.args.batch_size,
                chosen=self.chosen
            )
            if self.entropy_alpha > 0:
                # optional entropy augmentation omitted for brevity
                pass
            loss = self.actor.loss(best_actions, states, weights=w)
        else:
